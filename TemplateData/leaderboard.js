(function () {
  ...
  function writeCredentials(value) {
    credentials = value;
    try {
      window.localStorage.setItem("zineLeaderboardPlayer", JSON.stringify(value));
    } catch (_error) {
      // Private browsing can reject storage; the current page can still submit.
    }
  }
  ...
  function clearCredentials() {
    credentials = null;
    try {
      window.localStorage.removeItem("zineLeaderboardPlayer");
    } catch (_error) {
  ...
  async function api(path, options) {
    if (!apiBase) {
      throw new Error("랭킹 서버 주소가 설정되지 않았습니다.");
    }
  ...
  function authorizationHeaders() {
    return {
      "Authorization": "Bearer " + credentials.edit_token,
  ...
  async function createPlayer(nickname) {
    var player = await api("/api/v1/players", {
      method: "POST",
  ...
  async function saveNickname(nickname) {
    ...
    try {
      var updated = await api("/api/v1/players/me", {
        method: "PATCH",
      ...
      writeCredentials({
        ...
        edit_token: credentials.edit_token,
  ...
  openButton.addEventListener("click", function () {
    ...
    setStatus("");
    manuallyPaused = true;
    if (window.zineUnityInstance) {
      window.zineUnityInstance.SendMessage("Head", "PauseForLeaderboard");
    }
    showModal();
  });
  ...
  nicknameForm.addEventListener("submit", async function (event) {
    ...
    try {
      ...
      setStatus(result.nickname === "**" || result.nickname.indexOf("**") >= 0
        ? "비속어가 감지되어 ** 처리했습니다. 기록은 등록됐습니다."
        : "닉네임과 기록을 저장했습니다.", "success");
      await loadLeaderboard();
    } catch (error) {
      setStatus(error.message, "error");
    } finally {
